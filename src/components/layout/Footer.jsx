    ...
    <footer className="bg-slate-900 text-slate-300">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
          ...
          {/* Quick Links */}
          <div>
            <h4 className="text-lg font-semibold text-white mb-4">{t('common.quickLinks')}</h4>
          ...
          {/* Contact Info */}
          <div>
            <h4 className="text-lg font-semibold text-white mb-4">{t('common.contactUs')}</h4>
            <ul className="space-y-3">
              <li className="flex items-start space-x-3">
                <MapPin className="w-5 h-5 text-blue-400 mt-0.5 flex-shrink-0" />
                <span className="text-sm">{getText(schoolInfo.address)}</span>
              </li>
          ...
          {/* Timing */}
          <div>
            <h4 className="text-lg font-semibold text-white mb-4">{t('common.schoolHours')}</h4>
            <div className="flex items-start space-x-3">
              <Clock className="w-5 h-5 text-blue-400 mt-0.5 flex-shrink-0" />
              <span className="text-sm">{getText(schoolInfo.timing)}</span>
            </div>
            <div className="mt-4 p-4 bg-slate-800 rounded-lg">
              <p className="text-sm text-slate-400">
                {t('common.admissionsOpen')} {currentYear}-{currentYear + 1}
              </p>
              <Link
                to="/admissions"
        ...
        <div className="mt-12 pt-8 border-t border-slate-800 text-center text-sm text-slate-400">
          <p>© {currentYear} {getText(schoolInfo.name)}. {t('common.allRightsReserved')}</p>
        </div>
      </div>
    </footer>
  );
